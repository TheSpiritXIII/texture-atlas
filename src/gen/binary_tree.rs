...
impl<T> AsRef<Rect> for RectReference<T>
where
	T: AsRef<Rect>,
{
	fn as_ref(&self) -> &Rect {
...
fn sort_by_longest_width_increasing<T: AtlasRect>(
	atlas: &Atlas<T>,
	rotate: bool,
) -> Vec<RectReference<RotatableRect>> {
...
impl BinaryTreeGenerator {
	fn subdivide(leaves: &mut Vec<Rectr>, leaf_index: usize, width: u32, height: u32) {
		...
		let leaf_bottom =
			Rectr::new(leaf.bin, leaf.x, leaf.y + height, leaf.width, leaf.height - height);
			...
			for leaf_index in 0..leaves.len() {
				...
				if dimensions.width <= leaf.width && dimensions.height <= leaf.height {
					...
					atlas.bin_add_rect(
						...
						leaf.x,
						leaf.y,
						atlas_list[atlas_index].rect.rotated,
					);
